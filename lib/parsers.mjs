...
function deadlineNear(text, labels, zone) {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    for (const label of labels) {
      ...
      const parsed = parseExplicitDate(after, zone) ?? (/\|/.test(before) ? parseExplicitDate(before, zone) : null) ?? (/\|\s*$/.test(line) ? parseExplicitDate(lines[index + 1] || '', zone) : null);
...
function calendarRanges(text, expectedYear) {
  const result = [];
  const expression = new RegExp(`${MONTH}\\.?\\s+(\\d{1,2}),?\\s+(\\d{4})(?:\\s+@[^\\n]{0,40}?)?\\s+[–—-]\\s+${MONTH}\\.?\\s+(\\d{1,2}),?\\s+(\\d{4})`, 'gi');
  for (const match of text.matchAll(expression)) {
    if (+match[3] !== expectedYear || +match[6] !== expectedYear) continue;
    const startDate = ymd(+match[3], MONTHS.indexOf(match[1].slice(0, 3).toLowerCase()) + 1, +match[2]);
    const endDate = ymd(+match[6], MONTHS.indexOf(match[4].slice(0, 3).toLowerCase()) + 1, +match[5]);
    if (startDate && endDate && startDate <= endDate) result.push({ startDate, endDate });
  }
  return result;
}
...
export function parseConference(html, conference) {
  ...
  if (!['ICLR', 'CVPR', 'AAAI', 'ICRA', 'IROS', 'IV', 'ITSC'].includes(key)) return { patch, fields: [], mode: 'monitor', text, reason: '날짜 자동 해석을 지원하지 않아 페이지 변경을 감지합니다.' };
  ...
  deadline ??= deadlineNear(text, [/\bPaper submission deadline\s*:?/i, /\bFull paper(?:s)? (?:submission )?deadline\s*:?/i, /\bPaper deadline\s*:?/i, /^Submission\s+Deadline\s*[:|]?/i, /\bSubmission deadline for [^\n]{0,80}papers?\s*:?/i], globalZone);
  abstract ??= deadlineNear(text, [/\bAbstract(?:s)? (?:submission )?deadline\s*:?/i, /\bPaper\s+Registration\s+Deadline\s*[:|]?/i], globalZone);
  notification ??= deadlineNear(text, [/\bFinal Decisions?\s*:?/i, /\bAuthor Notification\s*:?/i, /\bConference Paper Notification Date\s*:?/i, /\bPaper Acceptance Notification\s*:?/i, /\bNotification of (?:Paper )?(?:Acceptance|Acceptance\/Rejection)\s*:?/i], globalZone);
  if (key === 'AAAI') {
  ...
  if (notification) {
    const dateYear = Number(notification.value.slice(0, 4));
    if (dateYear >= year - 1 && dateYear <= year) patch.notificationDate = notification.value;
  }
  ...
  } else if (['ICRA', 'IROS'].includes(key)) {
    const section = text.match(/will take place[\s\S]{0,260}/i);
    const event = (section ? ranges(section[0], year)[0] : null) ?? calendarRanges(text, year)[0];
    if (event) Object.assign(patch, event);
  } else if (['IV', 'ITSC'].includes(key)) {
    const event = ranges(text, year)[0] ?? calendarRanges(text, year)[0];
    if (event) Object.assign(patch, event);
  }
  const fields = Object.keys(patch).filter(field => ['deadline', 'abstractDeadline', 'notificationDate', 'startDate', 'endDate'].includes(field));
