...
function monitoredText(text, conference) {
  ...
  if (conference.monitorScope === 'year-line') {
    const year = String(conference.year ?? '');
    const matches = text.split('\n').map(line => line.trim()).filter(line => year && line.includes(year));
    return matches.length ? matches.join('\n') : `${conference.id}:year-not-announced`;
  }
...
export class ConferenceStore {
  ...
  async #refresh() {
    ...
    for (const conference of this.conferences) {
      const host = new URL(conference.monitorUrl ?? conference.sourceUrl).hostname;
    ...
    try {
      await pooled([...groups.values()], 3, async group => {
        for (const conference of group) {
          ...
          try {
            const { html, finalUrl } = await fetchSource(conference.monitorUrl ?? conference.sourceUrl, { fetchImpl: this.fetchImpl, timeout: this.timeout });
