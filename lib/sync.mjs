...
export class ConferenceStore {
  ...
  async initialize() {
    ...
    this.conferences = seed.conferences.map(conference => {
      ...
      const dynamic = Object.fromEntries(['deadline', 'abstractDeadline', 'deadlinePrecision', 'timezone', 'status', 'startDate', 'endDate', 'verifiedAt', 'sourceCheckedAt', 'sourceHash', 'syncStatus', 'syncMessage', 'syncMode', 'autoUpdatedFields', 'previousParsedFields', 'reviewRequired', 'lastChangedAt', 'sourceFinalUrl'].filter(key => key in saved).map(key => [key, saved[key]]));
      ...
      await pooled([...groups.values()], 3, async group => {
        for (const conference of group) {
          ...
          try {
            ...
            const sourceHash = hashText(result.text);
            ...
            const unverifiedKnownFields = ['deadline', 'abstractDeadline', 'startDate', 'endDate'].some(field => conference[field] && !result.fields.includes(field));
